...
class MainApp(tk.Tk):
    ...
    def create_widgets(self):
        self.bg_image = Image.open("assets/Pixelize .png")
    ...
    #     self.about_label.place(x=250, y=330)
        ...
        self.donate_button = tk.Button(self, text="DONATE", font=("Lucida fax", 15, "bold"), relief=tk.RAISED, borderwidth=5,command=self.donor_page)
        self.donate_button.config(bg="brown3")
        self.donate_button.place(x=100, y=550)
        self.donate_button.bind('<Double-1>', self.quit)

        self.need_button = tk.Button(self, text="NEED", font=("Lucida Fax", 15, "bold"), relief=tk.RAISED, borderwidth=5, command=self.acceptor_page)
        self.need_button.config(bg="brown3")
        self.need_button.place(x=320, y=550)
